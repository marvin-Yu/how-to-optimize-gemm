...
template <typename T>
void AddDot4x4(int, const T *, int, const T *, int, T *, int);
template <typename T>
void PackMatrixA(int, const T *, int, T *);
template <typename T>
void PackMatrixB(int, const T *, int, T *);
template <typename T>
void InnerKernel(int, int, int, const T *, int, const T *, int, T *, int, int);
...
template <typename T>
void InnerKernel(int m, int n, int k, const T *a, int lda,
                 const T *b, int ldb,
                 T *c, int ldc, int first_time)
{
  int i, j;
  T packedA[m * k];
  static T packedB[kc * nb]; /* Note: using a static buffer is not thread safe... */
...
template <typename T>
void PackMatrixA(int k, const T *a, int lda, T *a_to)
{
  int j;

  for (j = 0; j < k; j++)
  { /* loop over columns of A */
    const T *a_ij_pntr = &A(0, j);
...
template <typename T>
void PackMatrixB(int k, const T *b, int ldb, T *b_to)
{
  int i;
  const T
      *b_i0_pntr = &B(0, 0),
...
template <>
void AddDot4x4(int k, const float *a, int lda, const float *b, int ldb, float *c, int ldc)
{
  exit(0);
}

template <>
void AddDot4x4(int k, const double *a, int lda, const double *b, int ldb, double *c, int ldc)
{
